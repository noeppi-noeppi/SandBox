    ...
    return {
        'chunk_level_init': {
            ...
            'transformer': function (method) {
                ...
                target.add(new coremods_1.MethodInsnNode(coremods_1.Opcodes.INVOKESTATIC, 'io/github/noeppi_noeppi/mods/sandbox/impl/level/ChunkLevelInitializer', 'initChunkGenerator', '(Lnet/minecraft/world/level/chunk/ChunkGenerator;Lnet/minecraft/server/MinecraftServer;)V'));
                for (var i = 0; i < method.instructions.size(); i++) {
                    var node = method.instructions.get(i);
                    if (node.getOpcode() == coremods_1.Opcodes.INVOKEVIRTUAL) {
                        var methodNode = node;
                        if (methodNode.owner == 'net/minecraft/world/level/dimension/LevelStem' && methodNode.name == coremods_1.ASMAPI.mapMethod('m_63990_') && methodNode.desc == '()Lnet/minecraft/world/level/chunk/ChunkGenerator;') {
